...
class InstallerInnoTask < BaseTask
  def initialize(*args)
    super
    sandboxdir = sandbox_task.sandboxdir
    self.installer_exe = "installer-inno/rubyinstaller-#{package.rubyver_pkgrel}-#{package.arch}.exe"

    desc "installer for ruby-#{package.rubyver}-#{package.arch}"
    task "installer-inno" => [:devkit, "sandbox", installer_exe]

    filelist_iss = "installer-inno/filelist-ruby-#{package.rubyver}-#{package.ruby_arch}.iss"
    ...
    iss_files = Dir["installer-inno/*.iss"]
    file installer_exe => (sandbox_task.sandboxfiles + iss_files + [filelist_iss]) do
      sh "cmd", "/c", "iscc", "installer-inno/rubyinstaller.iss", "/Q", "/dRubyVersion=#{package.rubyver}", "/dRubyBuildPlatform=#{package.ruby_arch}", "/dRubyShortPlatform=-#{package.arch}", "/dDefaultDirName=#{package.default_instdir}", "/dPackageRelease=#{package.pkgrel}", "/O#{File.dirname(installer_exe)}", "/F#{File.basename(installer_exe, ".exe")}"
    end
  end
end
